  ...
  return (
    <div className="map-page">
      ...
      <main className="map-area">
        ...
        <div className={`map-overlay ${s.overlay}`}>
          <div className={s.overlayTitle}>Flugvellir</div>
          <div className={s.overlaySubtitle}>Plan your flight in Iceland</div>
          <p className={s.overlayDesc}>Airports · Weather · NOTAMs · Pilot notes</p>
          <div className={s.overlaySearchWrap}>
            <input
              ref={overlayInputRef}
              type="search"
              ...
              onBlur={() => setTimeout(() => setQuery(''), 150)}
            />
            <svg className={s.overlaySearchIcon} viewBox="0 0 16 16" fill="none" aria-hidden="true">
              <circle cx="6.5" cy="6.5" r="4.5" stroke="currentColor" strokeWidth="1.5"/>
              <line x1="10.5" y1="10.5" x2="14" y2="14" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
            </svg>
            {results.length > 0 && (
              <div className={`search-results is-visible ${s.overlayDropdown}`} role="listbox">
                {results.map((ap, i) => (
                  <button
                    key={ap.icao}
                    className={`search-result-item${i === activeIdx ? ' is-active' : ''}`}
